...
static void addChainSelectionOptions(AllowedArgs &allowedArgs)
{
    allowedArgs.addHeader(_("Chain selection options:"))
        .addArg("bfgtest", optionalBool, _("Use the btcforks genesis test chain"))  // MVF-BU
...
// MVF-BU begin add fork-specific help (MVHF-BU-DES-TRIG-8)
// used to be in src/mvf-bu.cpp:ForkCmdLineHelp()
static void addMVFOptions(AllowedArgs &allowedArgs)
{
    allowedArgs.addHeader(_("MVF-BU Forking Options:"))
         // automatic wallet backup parameters (MVHF-BU-DES-WABU-1)
        .addArg(
            "autobackupwalletpath=<path>", requiredStr, _("Automatically backup the wallet to the autobackupwalletfile path after the block specified becomes the best block (-autobackupblock). Default: Enabled"))
        .addArg("autobackupblock=<n>", requiredInt, _("Specify the block number that triggers the automatic wallet backup. Default: forkheight-1"))
        // fork height parameter (MVHF-BU-DES-TRIG-1)
        .addArg("forkheight=<n>", requiredInt, strprintf(_("Block height at which to fork on active network (integer). Defaults (also minimums): mainnet:%u,testnet=%u,nolnet=%u,regtest=%u,bfgtest=%u"), (unsigned)HARDFORK_HEIGHT_MAINNET, (unsigned)HARDFORK_HEIGHT_TESTNET, (unsigned)HARDFORK_HEIGHT_NOLNET, (unsigned)HARDFORK_HEIGHT_REGTEST, (unsigned)HARDFORK_HEIGHT_BFGTEST))

        // fork id (MVHF-BU-DES-CSIG-1)
        .addArg("forkid=<n>", requiredInt, strprintf(_("Fork id to use for signature change. Value must be between 0 and %d. Default is 0x%06x (%u)"), (unsigned)MAX_HARDFORK_SIGHASH_ID, (unsigned)HARDFORK_SIGHASH_ID, (unsigned)HARDFORK_SIGHASH_ID))

        // fork difficulty drop factor (MVF-BU TODO: MVHF-BU-DES-DIAD-?)
        .addArg("diffdrop=<n>", requiredInt, strprintf(_("Difficulty drop factor on active network (integer). Value must be between 1 (no drop) and %u. Defaults: mainnet:%u,testnet=%u,regtest=%u"), (unsigned)MAX_HARDFORK_DROPFACTOR, (unsigned)HARDFORK_DROPFACTOR_MAINNET, (unsigned)HARDFORK_DROPFACTOR_TESTNET, (unsigned)HARDFORK_DROPFACTOR_REGTEST))

        // option to enable special retargeting also on regtest (needed by fork
        // regression tests)
        .addDebugArg("force-retarget", optionalBool, _("Actively retarget on regtest network after fork (used by regression tests)"));
}
// MVF-BU end
