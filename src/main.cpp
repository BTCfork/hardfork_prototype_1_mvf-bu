...
void static UpdateTip(CBlockIndex *pindexNew)
{
    ...
    // if trigger block height reached or SegWit soft-fork activated, perform hardfork activation actions
    // (MVHF-BU-DES-TRIG-6)
    // MVF-BU TODO: the block height test condition below uses strict equality - check if correct
    // right now we haven't found a test case where >= would be needed, but we need to check if test coverage is
    // inadequate
    if (!isMVFHardForkActive &&
        ((chainActive.Height() == FinalActivateForkHeight) ||
                ...
                GetBoolArg("-segwitfork", DEFAULT_TRIGGER_ON_SEGWIT))))
    {
        ...
        // if preparations are only made after block has been accepted, then only FinalActivateForkHeight+1 can be new
        // rules
        ...
        else
        {
            ...
            if (chainActive.Height() == BackupBlock)
            {
                if (GetMainSignals().BackupWalletAuto(strWalletBackupFile, BackupBlock))
                    ...
                    // MVF-BU TODO: investigate if this is safe in terms of wallet flushing/closing or if more needs to
                    // be done
